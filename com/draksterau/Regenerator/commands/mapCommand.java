        ...
        if (command.plugin.getSenderPlayer(command.sender) == null) {
            ...
            for (x = (rootChunk.getX()-64); x <= (rootChunk.getX() + 64); x = x+16) {
                for (z = (rootChunk.getZ()-64); z <= (rootChunk.getZ() + 64); z = z+16) {
                    Chunk chunk = command.plugin.getSenderPlayer(command.sender).getLocation().getWorld().getChunkAt(x, z);
                    if (command.plugin.getCountIntegration(chunk) < 2) {
                        if (command.plugin.autoRegenRequirementsMet(chunk)) {
                            string = string + ChatColor.BLUE + "O";
                        } else {
                            if (command.plugin.canManuallyRegen(command.plugin.getSenderPlayer(command.sender), chunk) && command.plugin.getIntegrationForChunk(chunk) == null) {
                                string = string + ChatColor.GREEN + "#";
                            } else {
                                if (command.plugin.getIntegrationForChunk(chunk) != null) {
                                    if (command.plugin.getIntegrationForChunk(chunk).canPlayerRegen(command.plugin.getSenderPlayer(command.sender), chunk)) {
                                        string = string + ChatColor.GREEN + "*";
                                    } else {
