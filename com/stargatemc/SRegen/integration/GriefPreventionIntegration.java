...
public class GriefPreventionIntegration extends Integration {
    ...
    public boolean canPlayerRegen(Player player, Chunk chunk) {
        if (player.hasPermission("SRegen.regen.griefprevention.OWNER") && getOwnerOfChunk(chunk) == player || player.hasPermission("SRegen.regen.griefprevention.OVERRIDE")) {
            return true;
        } else {
            return false;
        }
    ...
    public String getPermissionRequiredToRegen(Player player, Chunk chunk) {
        if (getOwnerOfChunk(chunk) == player) {
            return ("SRegen.regen.griefprevention.OWNER");
        } else {
            return ("SRegen.regen.griefprevention.OVERRIDE");
        }
    }
