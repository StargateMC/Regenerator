...
public class WorldGuardIntegration extends Integration {
    ...
    // SRegen code for WG is based off: https://bukkit.org/threads/check-if-player-is-in-worldguard-region.29241/
    ...
    public boolean isChunkClaimed(Chunk chunk) {
        ...
        ProtectedCuboidRegion region = new ProtectedCuboidRegion("SRegenTEMP", pt1, pt2);        
    ...
    public void validateConfig() {
        ...
        for (String claimName : iConfig.integrationConfig.getStringList("claimsAutoRegen")) {
            if (!claimExists(claimName)) {
                SRegenPlugin.utils.throwMessage("warning", "[" + this.getClass().getSimpleName() + "] Claim: " + claimName + " does not exist!");
                SRegenPlugin.utils.disableIntegrationFor(SRegenPlugin.utils.convertToModule(plugin));
            } else {
                SRegenPlugin.utils.throwMessage("info", "[" + this.getClass().getSimpleName() + "] Claim: " + claimName + " detected. Whitelisting for automatic regeneration!");
            }
        }
    ...
    public String getPermissionRequiredToRegen(Player player, Chunk chunk) {
        if (isOwner(getRegionForChunk(chunk),player)) {
            return "SRegen.regen.worldguard.OWNER";
        } else {
            if (isMember(getRegionForChunk(chunk), player)) {
                return "SRegen.regen.worldguard.MEMBER";
            } else {
                return "SRegen.regen.worldguard.OVERRIDE";
            }
        }
    }
