 ...
 */
package com.stargatemc.SRegen.tasks;
...
public class regenTask extends BukkitRunnable {
    ...
    public regenTask (SRegen plugin) {
        this.plugin = plugin;
    }
    ...
    public void run() {
        ...
        if (!this.plugin.utils.isLagOK()) {
            plugin.utils.throwMessage("info", "SRegen is not beginning to parse inactive chunks as the TPS is below " + plugin.config.minTpsRegen + " (defined in global.yml).");
            return;
        }
        ...
        if (!chunksToRegenerate.isEmpty()) {
            ...
            plugin.utils.throwMessage("info", "SRegen will regenerate 1 chunk per " + secsBetweenChunks + " seconds for " + secsTotal + " seconds.");
