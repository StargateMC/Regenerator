...
public class infoCommand {
    ...
    public infoCommand(SRegenCommand SRegenCommand) {
        ...
        if (command.args.length == 2) {
            ...
            switch (command.args[1]) {
                ...
                case "chunk":
                    ...
                    if (rChunk.canManualRegen()) {
                        if (command.plugin.utils.canManuallyRegen(command.plugin.utils.getSenderPlayer(command.sender), chunk)) {
                            ...
                            if (command.plugin.utils.getIntegrationForChunk(chunk) == null) {
                                ...
                                command.sender.sendMessage(command.plugin.utils.getFancyName() + ChatColor.RED + "This requires the SRegen.regen.unclaimed permission node.");
                            } else {
                                ...
                                command.sender.sendMessage(command.plugin.utils.getFancyName() + ChatColor.RED + "Type /SRegen regen to find out what permission node is required.");
                            }
                        }
                    } else {
                    ...
                    break;
                default:
                    command.sender.sendMessage(command.plugin.utils.getFancyName() + ChatColor.RED + "Error.. You must specific either world or chunk! (eg. /SRegen info chunk)");
                    break;
            }
        } else {
            command.sender.sendMessage(command.plugin.utils.getFancyName() + ChatColor.RED + "Error.. You must specific either world or chunk! (eg. /SRegen info chunk)");
